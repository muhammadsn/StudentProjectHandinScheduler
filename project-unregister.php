...
if (isset($_REQUEST['id'])){
    if($project->fetch_project_info($_REQUEST['id'])){
        $student->delete_project($_REQUEST['id']);
        $student->set_message("پروژه با موفقیت حذف شد");
    }
    else{
        $student->set_message("حذف پروژه با خطا مواجه شد");
    }
}
else{
    $student->set_message("چیزی برای حذف وجود ندارد");
}
?>
